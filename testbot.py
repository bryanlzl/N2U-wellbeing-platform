...
def button(update: Update, _: CallbackContext):
    ...
    if query.data == "order":
        ...
        OrderP3_Keyboard = [[telegram.InlineKeyboardButton("⬅️ Back", callback_data = "order_generic")]]
        OrderP3_Markup = telegram.InlineKeyboardMarkup(OrderP3_Keyboard)
        query.edit_message_text(text=f"For this month's boxes, this is what you can expect to receive:\n\n<b>Box A :</b> Placeholder\n\n<b>Box B: </b> Placeholder", reply_markup = OrderP3_Markup, parse_mode = telegram.ParseMode.HTML)
    
    if query.data == "suggestions":
        SuggestionsHP_Keyboard = [[telegram.InlineKeyboardButton("Next Month Item Poll Nominations", callback_data = "suggestions_itempoll")],
                                  [telegram.InlineKeyboardButton("Poll Feedback on Mysterio Boxes", callback_data = "suggestions_feedbackpoll")],
        ...
        query.edit_message_text(text=f"Nice, we always appreciate your suggestions and your feedback! What would you like to discuss about?", reply_markup = SuggestionsHP_Markup)
    # Item Poll and Feedback Poll
        ...
        query.edit_message_text(text=f"<b>📞 Resources and Hotlines</b>\n\nNTU continues to support and empower students to take positive action through its various services.\n\n<b>NTU Counselling Centre (UCC)</b>\nBook an appointment: https://bit.ly/3cExnEN \n☎ Office Hours: +65 6790 4462\n☎ After-office Hours <i>(emergency only)</i>: +65 6904 7041\n📩 E-mail: ucc-students@ntu.edu.sg\n\n<b>Operating Hours</b>\nMonday - Thursday: 8:30am to 5:45pm\nFriday: 8:30am to 5:15pm\nWeekends and PH: Closed\n\n<b>External Resources</b>\n<i>SHINE Children & Youth Services</i>\n📱 WhatsApp: 8788 3171\n📩 E-mail: resilience@shine.org.sg \nWebsite: https://www.shine.org.sg/resilience", reply_markup = SuggestionsHPR_Markup, parse_mode = telegram.ParseMode.HTML)
    elif query.data == "suggestions_admin":
        SuggestionsHPR_Keyboard = [[telegram.InlineKeyboardButton("⬅️ Back", callback_data = "suggestions")]]
        SuggestionsHPR_Markup = telegram.InlineKeyboardMarkup(SuggestionsHPR_Keyboard)
        query.edit_message_text(text=f"Fantastic! We would love to get your feedback on our services in Telegram! Please do leave your suggestions and recommendations here with the command /feedback (message)\n\n<i>Please be aware that our administrators may contact you via Telegram to inquire about your suggestions where possible.</i>", reply_markup = SuggestionsHPR_Markup, parse_mode = telegram.ParseMode.HTML)
...
def command_TelegramboxA(update: Update, _: CallbackContext):
    ...
    _.bot.sendInvoice(chat_id = user.id, title = "NTU Mysterio Box A", description = "A motivational box for students (Placeholder)", payload = "swiftboxA", provider_token = "284685063:TEST:YWI4MTZhMmY3ZjYz", currency = "SGD", prices = [SwiftBoxA], max_tip_amount = 500, suggested_tip_amounts = [50, 100, 200, 500], photo_url = "https://i3.cpcache.com/product/1813695916/powerful_inspirational_quote_keepsake_box.jpg?side=Front&color=Mahogany&height=630&width=630&qv=90", photo_size = 200, photo_width = 200, photo_height = 200, need_name = True, need_phone_number = True, need_email = True, need_shipping_address = True, send_phone_number_to_provider = True, send_email_to_provider = True, is_flexible = True)

def command_TelegramboxB(update: Update, _: CallbackContext):
    user = update.message.from_user

    SwiftBoxA = LabeledPrice("NTU Mysterio Box B", 600)
    _.bot.sendInvoice(chat_id = user.id, title = "NTU Mysterio Box B", description = "A motivational box for students (Placeholder)", payload = "swiftboxB", provider_token = "284685063:TEST:YWI4MTZhMmY3ZjYz", currency = "SGD", prices = [SwiftBoxA], max_tip_amount = 500, suggested_tip_amounts = [50, 100, 200, 500], photo_url = "https://i3.cpcache.com/product/1813695916/powerful_inspirational_quote_keepsake_box.jpg?side=Front&color=Mahogany&height=630&width=630&qv=90", photo_size = 200, photo_width = 200, photo_height = 200, need_name = True, need_phone_number = True, need_email = True, need_shipping_address = True, send_phone_number_to_provider = True, send_email_to_provider = True, is_flexible = True)
...
def command_Telgrambox_shippingquery(update: Update, _: CallbackContext) -> None:
    ...
    options = [ShippingOption('1', 'Self Collection', [LabeledPrice('Self Collection', 0)]), ShippingOption('2', 'Mail-In Delivery', [LabeledPrice('Mail-In Delivery', 150)])]
